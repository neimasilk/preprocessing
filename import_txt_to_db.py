...
class ImportTxttoDB(object):
    def __init__(self, sqfile='parallelcorpusdata',tablename='id_zhcn',file_text1='id_text.txt',file_text2='zhcn_text.txt'):
    ...
    def import_txt_to_db(self):
        ...
        zhcn_text, panjang_zhcn = self.buka_file_teks(self.filetext2)

        if panjang_id==panjang_zhcn:
            for baris in range(panjang_id):
                # print(id_text[baris],zhcn_text[baris])
                #     loop panjang masukkan kedalam database dengan insert into
                try:
                    self._db_cur.execute("INSERT INTO id_zhcn(text_id, text_zhcn) VALUES (?,?)",(id_text[baris], zhcn_text[baris]))
...
if __name__ == '__main__':
    ...
    import_text = ImportTxttoDB('testparallel.db','id_zhcn','./opensubs2018/bitext.id','./opensubs2018/bitext.zh')
